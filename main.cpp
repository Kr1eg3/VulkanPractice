...
namespace std {
    template<> struct hash<Vertex> {
        size_t operator()(Vertex const& vertex) const {
            return ((hash<glm::vec3>()(vertex.pos) ^ (hash<glm::vec3>()(vertex.color) << 1)) >> 1) ^ (hash<glm::vec2>()(vertex.texCord) << 1);
        }
    };
}
...
private:
	...
	void setupDebugMessenger() {
		...
		if (CreateDebugUtilsMessengerEXT(m_instance, &createDebugInfo, nullptr,
...
int main() {
	...
	Logger::log(1, "%s : Success! Program run correctly!\n",
		__FUNCTION__);
